...
if NEW_NETWORK
    [e, n] = create_Tolman_B_Net_core(FIXED_WEIGHTS,10);
elseif ~exist('n')
...
% n.so('n_reward').setvals([14 21])=[0.2 0.8];
...
%     n.setNode('n_drives',[0.5, 1]); %Sets initial activity level of different drive
    ...
    for t = dt:dt:endTime
%         n.so('n_goal').vals([14 21])=[0.37 0.8]; n.so('n_goal').scaleVals(8);
            n.so('n_reward').vals([21])=[0.8];
        ...
        if (~isempty(find((e.curState == [14 21]))) && (tSinceMove > 2))
            disp('At goal')
            endTime = t;
