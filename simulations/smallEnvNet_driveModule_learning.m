...
%%%%%%%%%%%%%%%
NEW_NETWORK = 1;
SET_WEIGHTS = 0; %initialize core architecture weights to correct values
SET_GOAL_WEIGHTS = 0; %initialize drive -> goal weights to correct values
...
if NEW_NETWORK
    ...
    [e, n] = create_smallEnvNet_goalLearning(SET_WEIGHTS, SET_GOAL_WEIGHTS);
elseif ~exist('n')
...
% Run Settings %
%%%%%%%%%%%%%%%%
MAX_TIME = 100; %maximum trial length
...
%     n.gatePlot('p_drives_goal','',50);
    ...
    n.plot({'state', 'drives', 'adjacent', 'goal', 'next', 'gradient', 'n_drives','p_drives_goal'}, [dt,200]); %choose next state
