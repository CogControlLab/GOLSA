% runs TOLMAN-state environment simulation and transition revaluation
 warning('off','all')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%NOTES:: learn_type==3 worked, 4= worked, but slower?
%%%%%%%%%%%%%%%
% Basic Setup %
%%%%%%%%%%%%%%%
clear all
...
ERROR = false;
% 
% if or(learn_type==6,learn_type==7)
%     FIXED_WEIGHTS=0;
% end

if NEW_NETWORK
    [e, n] = create_Tolman_B_Net_core(FIXED_WEIGHTS,learn_type);
elseif ~exist('n')
...
%% make some copies of trained systems

%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%Hard drop the weights as imputing a barrier %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

MAX_TIME = 500; %maximum trial length
MAX_STUCK_TIME = pi*4 ; %maximum time to have a goal and not move
NUM_TRIALS = 2;
TEXT_OUTPUT = 1;
DISPLAY = 0;
n.so('p_state_adjacent').weights(19,20)=0;
n.so('p_state_adjacent').weights(20,19)=0;

n.so('p_gradient_gradient').weights(19,20)=0;
n.so('p_gradient_gradient').weights(20,19)=0;
e.transitions(40:41,:)=[];
