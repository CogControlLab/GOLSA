function [e, n] = create_Tolman_GOALS_Net_core(set_weights,set_goal_weights,drive_const)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%% UNCOMMENT 40 or 41 and 147 to get a representation that suppresses
%%%% GOAL on each state change.
...
rewardVec = [1,14; 2,21;3,22]; %col1: reward type; col2: state
e=stateSpace(Tolman_A, 1,rewardVec); %(transitionMatrix, starting location)
...
n=network();
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    n.addLayer('drives',{numDrives,'shunt', drive_const, 2}); %Time constant will have a dramatic effect on when low activity begins to rise.
    n.so('drives').bias = -.1;

    n.addLayer('goal',{numStates,'easyInhib',.5,0});
     n.so('goal').set_inhibitors({{'n_novInhib',[.01, 1, 0]}});
%      n.so('goal').set_inhibitors({{'n_GoalInhib',[.01, 1, 0]}});
    ...
    n.so('n_novInhib').updateFunc = @(x) max(x - (dt/2), 0);
     %Inhibition following a goal achievement
    n.addNode('n_GoalInhib', 1);
    n.so('n_GoalInhib').updateFunc = @(x) max(x - (dt/2), 0);
  ...
  n.connect('goal', 'goal', 'imposeStrong', 0, 4, 'null', 0); % commenting this out would in theory remove suppression
    ...
    n.so('p_next_transDes').set_weights(transToWeights);
     ...
     n.connect('transDes', 'transDes', 'imposeStrong', 0, 2, 'null', 0); % was 2
