function [e, n] = create_hanoiNet_core(set_weights)
    ...
    e=stateSpace(hanoiSpace, 1); %(transitionMatrix, starting location)
    ...
    n.addLayer('gradient2', {numStates, 'easyInhib', 1, 0.2}); 
    n.so('gradient').set_inhibitors({{'n_novInhib', [.8, 1, 0], -8}, {'n_subOsc', [-.2, .2, 0]}});
    n.so('gradient2').set_inhibitors({{'n_novInhib', [.8, 1, 0], -8}, {'n_subOsc', [-.2, .2, 0]}});
    ...
    n.connect('goal', 'gradient2', 'one-to-one', 0, 4, 'null', 0, {{'n_subOsc', [-.2, -1, 0]}});  

    n.connect('gradient', 'gradient2', 'one-to-one', 0, 1, 'gradient', 15);
    n.so('p_gradient_gradient2').learnGate={{n.so('n_subOsc'), [.55, 1]}};
    n.so('p_gradient_gradient2').learningParams = [.98, .85, .98];
    ...
    n.connect('gradient2', 'gradient2', 'one-to-one', 0, 1, 'gradient', 15);
    n.so('p_gradient2_gradient2').learnGate={{n.so('n_subOsc'), [.55, 1]}};
    n.so('p_gradient2_gradient2').learningParams = [.98, .85, .98];
