function [e, n] = Hanoi_GOALS_Net_core(set_weights,set_goal_weights,verst)
G_G=4;
bgmot=0.6;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
global dt
    ...
    load('hanoiSpace.mat');
    load('hanoiSpace_weights.mat'); 



rewardVec = [1,20; 2,27]; %col1: reward type; col2: state
e=stateSpace(hanoiSpace, 1,rewardVec); %(transitionMatrix, starting location)
...
if verst==1
    ...
    n.addLayer('drives',{numDrives,'shunt', 0.4, 2});
    n.so('drives').bias = -.1;
    ...
    n.addLayer('gradient', {numStates, 'easyInhib', 1, 0.5}); 
    ...
    bgFunc = @(vals, in) (any(vals>0)*max((vals-(dt/5)), 0))+(all(vals<=0).*(in>=bgmot).*(in==max(in)));
    ...
    %== Choosing a goal ==
    %=====================
    %Learnign weights between drives and goal--states
    driveGoalWeightFactor = 5;
    ...
    n.connect('goal', 'goal', 'imposeStrong', 0, 6, 'null', 0); % commenting this out would in theory remove suppression
    ...
    n.connect('goal', 'gradient', 'one-to-one', 0, 4, 'null', 0, {{'n_subOsc', [-.2, -1, 0]}});  %wweight factor was 4
...
elseif verst==2
    ...
    n.connect('goal', 'gradient', 'one-to-one', 0, G_G, 'null', 0, {{'n_subOsc', [-.2, -1, 0]}});  %wweight factor was 4
