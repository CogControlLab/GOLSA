...
if verst==1
    ...
    n.addLayer('reward', {numStates, 'shunt', .9, 1}) % OK - could change .9 to something lower
    n.so('reward').noiseGain=0.05; % OK
...
elseif verst==8 % for learning and operating on multiple goals, uses WTA strong
    %=============================
    %== Choosing the next state ==
    %=============================
    ...
    n.addLayer('state', {numStates, 'shunt', .5, 1});
       ...
       n.addLayer('goal',{numStates,'easyInhib',1,1});
%      n.so('goal').set_inhibitors({{'n_novInhib',[.01, 1, 0]}});

%      n.addLayer('goal', {numStates, 'shunt', 1, 1});
     n.so('goal').noiseGain = .3;
    ...
    n.connect('goal', 'gradient', 'one-to-one', 0, 6, 'null', 0, {{'n_subOsc', [-.2, -1, 0]}});
    ...
    n.connect('next', 'next', 'impose', 0, 2, 'null', 0);
    n.connect('goal', 'goal', 'imposeStrong', 0, 4, 'null', 0);
...
elseif verst ==9 %uses weak/no competition between goals in recurrent connections
    ...
    n.connect('next', 'next', 'impose', 0, 2, 'null', 0);
    n.connect('goal', 'goal', 'impose', 0, 2, 'null', 0);
...
elseif verst ==10 %replaces goal with/reward layer
     %=============================
    %== Choosing the next state ==
    %=============================
    ...
    n.addLayer('state2', {numStates, 'shunt', .5, 1});
     ...
     n.addLayer('reward', {numStates, 'shunt', .9, 1});
...
%     n.connectNode('n_goal', 'goal');
    ...
    n.connect('reward', 'gradient', 'one-to-one', 0, 4, 'null', 0, {{'n_subOsc', [-.2, -1, 0]}});
    ...
    n.connect('next', 'next', 'impose', 0, 2, 'null', 0);
    n.connect('reward', 'reward', 'impose', 0, 2, 'null', 0); %ADDED
...
% %     n.so('p_state_reward').learningParams = [.6, .7, .6];
    ...
    %V_A
       n.so('p_gradient_next').addMod(n.so('reward'))  % TESTING
